...
### 8.1 子 agent 清單與單一 Agent 架構

架構原則：**對話與語音統一由單一 ElevenLabs Agent 處理**（對話生成＋TTS 一體，不再做 DeepSeek／強模型雙軌路由）。評估總結、課程設計師、Avatar 調度器的產出是給家長的文字／JSON 配置，由規則庫（離線兜底）或同一 agent 內部生成，前端不感知走了哪條路。

| 子 agent       | 系統提示          | 職責                         | 輸出契約                      | 處理方式 |
| ------------- | ------------- | -------------------------- | ------------------------- | ---- |
